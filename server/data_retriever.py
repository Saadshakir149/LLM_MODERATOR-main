...
# Fixed UI labels for the task card, per supported language (English / Roman Urdu).
# Item names + the scenario description come from task_data and are NOT translated.
_INTRO_LABELS = {
    "en": {
        "items_heading": "Items in your packs",
        "task_heading": "Your task",
        "step_rank": "Rank items 1 (most important)–12 (least important).",
        "step_agree": "Agree on one final group ranking.",
        "step_explain": "Be ready to explain your reasoning briefly.",
        "time": "⏰ 15 minutes to reach consensus.",
    },
    "roman_urdu": {
        "items_heading": "Aap ke pack mein cheezein",
        "task_heading": "Aap ka kaam",
        "step_rank": "Cheezon ko 1 (sab se zaroori) se 12 (sab se kam zaroori) tak rank karein.",
        "step_agree": "Ek final group ranking par sab mutfiq hon.",
        "step_explain": "Apni wajah mukhtasar batane ke liye tayyar rahein.",
        "time": "⏰ Consensus tak pohanchne ke liye 15 minute.",
    },
}


def get_story_intro_html(task_data: dict, language: str = "en") -> str:
    """Sanitized HTML task card for chat (renders with trusted server-only markup).

    `language` selects the fixed UI labels ("en" or "roman_urdu"); item names and
    the scenario description come from task_data unchanged.
    """
    labels = _INTRO_LABELS.get(language, _INTRO_LABELS["en"])
  ...
  <h4 class="font-semibold text-gray-800 mb-2 text-sm">{labels["items_heading"]}</h4>
  ...
  <div class="mt-2 p-3 bg-amber-50 rounded-lg border border-amber-200 text-sm">
    <p class="font-semibold text-amber-900 mb-2">{labels["task_heading"]}</p>
    <ol class="list-decimal pl-5 space-y-1 text-gray-800">
      <li>{labels["step_rank"]}</li>
      <li>{labels["step_agree"]}</li>
      <li>{labels["step_explain"]}</li>
    </ol>
    <p class="text-amber-800 mt-2 font-medium">{labels["time"]}</p>
  </div>
</div>"""
